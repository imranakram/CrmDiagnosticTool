        ...
        private void BuildSqlInformation(HtmlGenericControl main)
        {
            if (this.settings.UseSqlSqlServerInformation)
            {
                ...
                try
                {
                    ...
                    DataTable data2 = DatabaseHelper.ExecuteQuery(configConnectionString,
                                                                  "EXEC sp_configure 'show advanced options', 1 RECONFIGURE WITH OVERRIDE exec sp_configure 'max degree of parallelism' RECONFIGURE WITH OVERRIDE EXEC sp_configure 'show advanced options', 0");

                    this.rcb.AddTabHeader(gc, "Max degree of parallelism");
                    this.rcb.AddTable(gc, data2);
